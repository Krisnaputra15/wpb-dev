...
@section('content')
    ...
    <div class="container-fluid h-100">
        ...
        <div class="py-3 py-lg-4">
            <div class="row">
                <div class="col-lg-6">
                    <h3 class="page-title mb-0">Edit Pilihan Booth </h3>
                </div>
                {{-- <div class="col-lg-6">
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body py-2 px-4">
                    ...
                    <form action="{{ route('boothTransaction.updateBooth', [$id]) }}" method="POST" id="checkout-form">
                        @csrf
                        @method('PUT')
                        <button type="submit" id="submit-btn"
                            class="btn btn-primary btn-block w-100 mt-4 mb-2 d-none">Perbarui Booth</button>
                    </form>
                </div>
            </div>
        </div>
                            ...
                            <div class="mb-2" id="booth-container">
                                ...
                                <div id="booths" class="d-flex justify-content-center">
                                    <img src="{{ asset('images/master/layoutBg.png') }}" class="responsive-img"
                                        style="margin-top: 8rem !important;" alt="" width="550" height="730">
...
@push('script')
    <script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    for (var i = 0; i < 33; i++) {
                        ...
                        for (var j = 0; j < 33; j++) {
                            baseLayout +=
                                `<div class="booth-cell booth-selection d-flex justify-content-center align-items-center booth-padding unclickable" id="${i+1}_${j+1}" style="--bs-border-opacity: .1;" data-booth="" data-pov=""></div>`;
                        }
                        baseLayout += `</div>`;
                    }
                    ...
                    response.data.forEach(booth => {
                        ...
                        positionIds.forEach(posId => {
                            ...
                            if (isBottom) $cell.addClass('border-black').addClass(
                                'border-bottom');
                            ...
                            $cell.attr('style', `background-color:${booth.color}`)
                                    .attr('data-booth', booth.id)
                                    .attr('data-pov', booth.booth_pov_file == null || booth
                                        .booth_pov_file == '' ? '' : 'storage/'+booth.booth_pov_file)
        ...
        $(document).ready(function() {
            fetchBoothLayout(@json($boothTransaction->layout_id));
            ...
            $(document).on('click', '.booth-selection', function() {
                ...
                if ($(`.booth-selection[data-booth="${databooth}"]`).hasClass('selected')) {
                    ...
                    $(`.booth-selection[data-booth="${databooth}"]`).addClass('selected').addClass(
                        'bg-success');
                    const list = `
            ...
            $(document).on('click', '.pov', function() {
                ...
                if (pov != null || pov != '') {
                    $('#sky').attr('src', '{{ asset(':asset') }}'.replace(":asset", $(this).attr(
                        'data-pov')));
            ...
            $('#submit-btn').click(function() {
                ...
                selectedBooth.forEach(value => {
                    $('#checkout-form').append(
                        `<input type="hidden" name="registered_booth_id[]" value="${value}">`);
                });
