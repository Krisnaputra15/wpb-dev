...
@section('content')
    ...
    <div class="container-fluid">
        <div class="py-3 py-lg-4">
            <div class="row">
                <div class="col-lg-6">
                    <h3 class="page-title mb-0">Daftar Transaksi</h3>
                </div>
                {{-- <div class="col-lg-6">
                ...
                <div class="card">
                    <div class="card-body">
                        <form id="search-form" class="d-flex flex-row mb-4 justify-content-between gap-2 w-100">
                            ...
                            <select name="bt.status" id="bt.status" class="form-select">
                                @foreach ($status as $key => $data)
                                    <option selected disabled hidden>Status Transaksi</option>
...
@push('script')
    <script>
        ...
        function loadTransaction(page, isNew){
            ...
            $.ajax({
                ...
                success: function(response){
                    haveMorePage = response.data.next_page_url == null ? false : true;
                    if(response.data.data.length == 0){
                        $('#loader').hide();
                        $('.list-group').html(`<h4 class="text-center my-auto">Tidak ada transaksi yang terdaftar</h4>`)
                        isLoading = false;
                        return;
                    }

                    let receivers = response.data.data.map((transaction, index) => {
                        let url = transaction.link;
                        ...
                        return `<div class="receiver list-group-item py-3 d-flex flex-row gap-2 w-100">
                                    ...
                                    <div class="d-flex flex-row justify-content-between align-items-center" style="width: 97% !important">
                                        <div class="d-flex flex-column gap-2 justify-content-between">
                                            <h5 class="ps-4 m-0">${transaction.agenda_name}</h5>
                                            <div class="ps-4 d-flex flex-row gap-1">
                                                <div>
                                                    <p class="m-0 fw-semibold">Tanggal transaksi</p>
                                                    <p class="m-0 fw-semibold">Perusahaan</p>
                                                    <p class="m-0 fw-semibold">Status</p>
                                                </div>
                                                <div>
                                                    <p class="m-0">: ${transaction.created_at}</p>
                                                    <p class="m-0">: ${transaction.company_name}</p>
                                                    ${transaction.status}
                                                </div>
                                            </div>
                                            <h5 class="ps-4 mt-1">Total : Rp${transaction.total_price}</h5>
                                        </div>
                                        <a href="${url}" class="btn btn-primary btn-sm h-25 my-auto">Lihat Detail</a>
                                    </div>
                                </div>`;
                    }).join('');
        ...
        $(window).scroll(function() {
            if (haveMorePage && (window).scrollTop() + $(window).height() >= $(document).height() - 100) {
